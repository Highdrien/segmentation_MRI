...
if __name__ == "__main__":
    ...
    #
    # print(L)
    # res=load_nii('../training/patient001/patient001_frame01.nii.gz')
    # #print(res)
    ...
    y = []
    z = []
    for patient in os.listdir('../training'):
        # print(patient)
        if patient[-1] != 'x' and patient[-1] != 'y':
            # print('patient=',patient)
            path_patient = os.path.join('../training', patient)
            list_of_path = os.listdir(path_patient)
            for frame in list_of_path:
                if len(frame) == 25:
                    frame_path = os.path.join(path_patient, frame)
                    # print(frame_path)
                    shape = np.shape(load_nii(frame_path)[0])
